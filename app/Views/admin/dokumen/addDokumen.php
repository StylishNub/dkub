...
<div class="max-w-3xl mx-auto mt-12 px-6 py-8 bg-white dark:bg-gray-900 shadow-md rounded-xl">
    ...
    <!-- Judul -->
    <h1 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">Tambah Dokumen</h1>
    ...
    <!-- Form Tambah Dokumen -->
    <form action="/tambah_dokumen/save_dokumen" method="post" enctype="multipart/form-data" class="space-y-6">
        <!-- Nama Dokumen -->
        <div>
        ...
        <div>
            ...
            <select name="jenis_dokumen_id" id="jenis_dokumen_id" required
                ...
                <?php foreach ($jenis as $s): ?>
                    <option value="<?= esc($s['id']); ?>"><?= esc($s['jenis']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
